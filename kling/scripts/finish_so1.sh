#!/usr/bin/env bash
# Post-pro batch SO1 (personificacion Pixar, sin lipsync):
# assemble (voz + clips Grok, cortes por cuts) + subs karaoke + headline 1a toma + musica 12%.
# USO: bash finish_so1.sh so1_1 [so1_2 ...]
cd /d/videos-kling
...
run_finish(){
  ...
  node assemble_voiced.mjs $ad audio/SO1/${ad}_full.mp3 clips/$ad clips/${ad}_sync config/cuts_${ad}.json 2>&1 | grep -E "final_|❌|toma"
  ...
  "$FF" -nostdin -v error -y -i "output/final_${ad}_voiced.mp4" -i "$HL" -i "$MUSIC" \
    ...
    -map "[v]" -map "[a]" -c:v libx264 -preset medium -crf 18 -c:a aac -b:a 192k "output/SO1/${ad}.mp4"
  echo "  ${ad}.mp4 rc=$? -> $(ffprobe -v error -show_entries format=duration -of csv=p=0 output/SO1/${ad}.mp4 2>/dev/null)s"
}
for ad in "$@"; do run_finish "$ad"; done
echo SO1_FINISH_DONE
