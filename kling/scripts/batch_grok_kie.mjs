 ...
 * batch_grok_kie.mjs
 * ─────────────────────────────────────────────────────────────────────────────
 * Igual que batch_kling3_kie.mjs pero usando GROK IMAGINE (xAI) image-to-video en KIE.AI.
 * Mucho más barato que Kling: 720p = 3 cr/s ($0.015/s) · 480p = 1.6 cr/s ($0.008/s).
 * Mismo formato de batch_input.json → intercambiable con el pipeline Kling/fal.
 *
 * USO
 *   node batch_grok_kie.mjs [batch_input.json] [clips_dir] [resolution] [mode]
 *   resolution: "720p" (default) | "480p"
 *   mode:       "normal" (default) | "fun" | "spicy"
 *
 * FORMATO batch_input.json  (mismo que Kling)
 *   [ { "n":"01", "image_url":"https://…", "prompt":"…", "duration":6 } ]
 *   OJO: Grok exige duration 6–30 s (mínimo 6, NO 5). <6 se sube a 6 automáticamente.
 *
 * API KIE.AI:
 *   - POST https://api.kie.ai/api/v1/jobs/createTask
 *       body: { model:"grok-imagine/image-to-video",
 *               input:{ prompt, image_urls:[url], duration:6, resolution:"720p",
 *                       aspect_ratio:"9:16", mode:"normal" } }
 *       → data.taskId
 ...
 * SALIDA: <clips_dir>/raw_<n>.mp4 + <clips_dir>/batch_log_grok.json
 * ─────────────────────────────────────────────────────────────────────────────
 */
...
const CR_PER_SEC = { "480p": 1.6, "720p": 3 };
...
const clipsDirName = process.argv[3] ?? "clips/grok";
const RES = (process.argv[4] ?? "720p").toLowerCase();
const MODE = (process.argv[5] ?? "normal").toLowerCase();
if (!CR_PER_SEC[RES]) { console.error(`\n❌  resolution debe ser 480p o 720p (recibido: ${RES}).\n`); process.exit(1); }
...
for (const s of shots) {
  ...
  let d = Number(s.duration ?? 6);
  if (!Number.isFinite(d)) { console.error(`\n❌  Toma ${s.n}: duration inválida.\n`); process.exit(1); }
  if (d < 6) d = 6;                 // Grok exige mínimo 6 s
  if (d > 30) d = 30;
  s.duration = Math.round(d);
}
...
let estCredits = 0;
console.log(`\n${SEP2}`);
console.log(`  BATCH GROK IMAGINE i2v vía KIE.AI — ${RES} / mode:${MODE}`);
console.log(`  Archivo: ${basename(inputFile)}   |   Tomas: ${shots.length}   |   Salida: ${clipsDirName}/`);
console.log(SEP2);
console.log(`  ${"N°".padEnd(5)}${"Dur.".padEnd(6)}${"~Costo".padEnd(10)}Prompt animación`);
console.log(SEP);
for (const s of shots) {
  const cr = s.duration * CR_PER_SEC[RES]; estCredits += cr;
  console.log(`  ${String(s.n).padEnd(5)}${(s.duration + " s").padEnd(6)}${("$" + (cr * CREDIT_USD).toFixed(3)).padEnd(10)}${truncate(s.prompt, 50)}`);
}
console.log(SEP);
console.log(`  Estimado: ${estCredits} créditos ≈ $${(estCredits * CREDIT_USD).toFixed(3)}  (costo REAL = creditsConsumed al terminar)`);
console.log(`${SEP2}\n`);
...
console.log(`\n🚀  Enviando ${shots.length} job(s) a KIE.AI (Grok Imagine i2v, ${RES})…\n`);
...
writeFileSync(join(clipsDir, "batch_log_grok.json"), JSON.stringify({
  provider: "kie.ai", model: MODEL, resolution: RES, mode: MODE, started_at: startedAt.toISOString(),
  ended_at: new Date().toISOString(), total_credits: totalCredits, total_usd: totalUsd, shots: logEntries,
}, null, 2));

console.log(`\n${SEP2}`);
console.log("  LOTE GROK (KIE.AI) COMPLETADO");
console.log(SEP);
